...
void bubbleSortBarang()
{
	int swapped;
	struct nodeBarang *ptr1;
	struct nodeBarang *lptr = NULL;
	...
	do
	{
		...
		{
			if (ptr1->data.jumlahBarang > ptr1->next->data.jumlahBarang)
			{
				struct barang tempBarang = ptr1->data;
				ptr1->data = ptr1->next->data;
				ptr1->next->data = tempBarang;

				time_t tempDibuat = ptr1->waktuDibuat;
				ptr1->waktuDibuat = ptr1->next->waktuDibuat;
				ptr1->next->waktuDibuat = tempDibuat;

				time_t tempDiubah = ptr1->waktuDiubah;
				ptr1->waktuDiubah = ptr1->next->waktuDiubah;
				ptr1->next->waktuDiubah = tempDiubah;

				swapped = 1;
			}
			ptr1 = ptr1->next;
		}
		lptr = ptr1; // nentuin akhir biar algoritma ga perlu mengulang apa yang sudah benar.
	} while (swapped);
}
